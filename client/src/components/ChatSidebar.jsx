...
export default function ChatSidebar({ 
    ...
    user, 
    unreadChats,
    typingUsers // <--- Received from Chat.jsx
}) {
    return (
        <div className="flex flex-col h-full bg-white/80 backdrop-blur-xl border-r border-gray-200/50">
            ...
            <div className="overflow-y-auto flex-1 p-2 space-y-1 custom-scrollbar">
                {conversations.length === 0 ? (
                    ...
                    conversations.map((c) => {
                        // Check if this specific friend is typing
                        ...
                        return (
                            <div 
                                ...
                                ${currentChat?._id === c._id 
                                    ? 'bg-indigo-50 border-l-4 border-uni shadow-sm' 
                                    : 'hover:bg-white/60 hover:shadow-md'}`}
                            >
                                <Conversation 
                                    conversation={c} 
                                    currentUser={user} 
                                    isTyping={isFriendTyping} // <--- Pass Typing State
                                />
                                
                                {unreadChats.includes(c._id) && currentChat?._id !== c._id && (
                                    <div className="bg-uni text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md animate-pulse border border-white">
                                        NEW
                                    </div>
                                )}
                            </div>
